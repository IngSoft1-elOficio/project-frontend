...
describe('HideRevealStealSecrets', () => {
  ...
  it('muestra nombre y efecto del detective correctamente', () => {
    ...
    expect(
      screen.getByText(/Elige un secreto del jugador objetivo para revelar/)
    ).toBeInTheDocument();
  });
  ...
  it('filtra los secretos solo del jugador objetivo', () => {
    ...
    expect(cards).toHaveLength(2); // Solo los del playerId 10
  });
  ...
  it('deshabilita el botón confirmar si no hay secreto seleccionado', () => {
    ...
    // El botón debería estar deshabilitado al inicio
    expect(button).toBeDisabled();
    // Y no debería llamar a onConfirm si se clickea
  ...
  // ==== DETECTIVE PYNE (oculta secretos revelados) ====
  ...
  it('maneja detective desconocido', () => {
    const unknown = { ...defaultDetective, current: { setType: 'Desconocido' } };
